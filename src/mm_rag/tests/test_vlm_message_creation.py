from mm_rag.agents.flows import formatter, State
...
class TestMessageCreation(unittest.TestCase):
  ...
  def test_correct_img_message_creation(self):
    ...
    for file_type in supported_types:
      with self.subTest(file_type=file_type):
        ...
        formatted = formatter(mock_state)  # type: ignore
  ...
  def test_missing_retrieval(self):
    ...
    missing_retrieval_info = formatter(mock_state)  # type: ignore
        ...
        expected_error_message_prompt = {
            "messages": {
              ...
              "content": [
                {
                  "type": "text",
                  "text": f"""Unfortunately, there was a problem during the formatting of the retrieved documents. The retrived docs did not have {'doc_id' if not doc.id else 'doc_type'}. Please inform the user and ask if he would like you to try again or respond without retrieval.""".strip()
                }
              ]
            }
          }
