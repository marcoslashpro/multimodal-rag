...
class Retriever(BaseRetriever):
  ...
  def transform_response_to_docs(self, pinecone_response: 'QueryResponse') -> list[Document]:
    docs: list[Document] = []

    if not isinstance(pinecone_response, QueryResponse):
      raise ValueError(
        f"Expected response to be of type `QueryResponse`, got instead: {type(pinecone_response)}"
      )
